	...
	denz.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*_ꜱᴀʏᴏɴᴀʀᴀ👋_* @${num.split('@')[0]} ꜱᴇᴍᴏɢᴀ ᴛᴇɴᴀɴɢ ᴅɪ ᴀʟᴀᴍ ꜱᴀɴᴀ`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           denz.sendMessage(from, options, text, { quoted: mek } )
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					denz.sendMessage(from, tod, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
					await limitAdd(sender) 
					break 
                case 'speed':
                case 'ping':
			...
			reply(mess.wait)
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						denz.sendMessage(from, bufferhgf, sticker, {quoted: mek})
						fs.unlinkSync(rano)
					})
					await limitAdd(sender)
					break  
...
// premium user
       ...
       case 'ytmp4':
					...
					denz.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.result.title}.mp4`, quoted: mek, caption: 'Nih Gan'})
					await limitAdd(sender) 
					break 
				...
				case 'ytmp3':
					...
					denz.sendMessage(from, buff, image, {quoted: mek, caption: yta})
					denz.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: mek, caption: 'Nih Gan'})
					await limitAdd(sender) 
					break 
...
denz.sendMessage(from, hasil, video, {
...
case 'husbu':
			...
			break
				default:
                  ...
                  reply(`*ᴍᴀᴀғ ᴋᴀᴋ, ᴄᴏᴍᴍᴀɴᴅ ${prefix}${command} ᴛɪᴅᴀᴋ ᴛᴇʀᴅᴀғᴛᴀʀ ᴅɪ ᴅᴀʟᴀᴍ ᴅᴀᴛᴀʙᴀꜱᴇ* ${prefix}menu`)
                  }
